...
function Connection(manager, options, req, socket, upgradeHead) {
  ...
  this._options = Mixin({
    version: 'auto',    // String: Value must be either: draft75, draft76, draft10, draft16, auto
    ...
    version: {
      get: function() {
		if (req.headers['sec-websocket-version']){
			switch(req.headers['sec-websocket-version']){
				case "8":
					return "draft10";
				case "13":
					return "draft16";
  ...
  if (!checkVersion(this)) {
    ...
    // Handle close event, as 'end' is not always fired
    socket.on('close', function() {
        debug(connection.id, 'close');
        connection.state(5);
    });
...
Connection.prototype.write = function(data) {
  if (this._state === 4) {
    if (this.version == 'draft10' || this.version == 'draft16') {
      var byteLen = Buffer.byteLength(data, 'utf8'),
          bytes;

      if (byteLen > 125) {
        bytes = new Buffer(byteLen + 4);
      } else {
      ...
      if (byteLen > 125) {
        bytes[1] = 126;
        bytes[2] = byteLen >> 8;
        bytes[3] = byteLen & 0xFF;
        bytes.write(data, 4, 'utf8');
      } else {
        bytes[1] = byteLen;
        bytes.write(data, 2, 'utf8');
      }
...
-----------------------------------------------*/
var handshakes = {
  draft10: function(connection) {
    var key = Crypto.createHash('sha1');
    key.update(connection._req.headers['sec-websocket-key']);
    key.update('258EAFA5-E914-47DA-95CA-C5AB0DC85B11');
  ...
  for (var i = 0, len = data.length; i < len; i++) {
    ...
    if (this.order == -1) {
      ...
      if (this.closing && data[i] === 0x00) {
        ...
        if (this.length <= this.frameData.length) {
          pkt = new Buffer(this.frameData.length);

          for (var j = 0, len2 = this.frameData.length; j < len2; j++) {
            pkt[j] = this.frameData[j] ^ this.mask[j % 4];
          }
